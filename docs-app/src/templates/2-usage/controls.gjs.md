...
`@universal-ember/form` comes with support for the following controls built-in (but you can also use [custom controls](/2-usage/custom-controls.md)), all yielded from the `Field` component:
...
<template>
  <HeadlessForm as |form|>
    <form.Field @name="payment_method" as |field|>
