...
In the most basic way validation will happen whenever you try to submit a form. This is how server-rendered apps without any native validation or JavaScript worked for a long time. And in fact, `@universal-ember/form` enforces validation whenever you try to submit the form.
